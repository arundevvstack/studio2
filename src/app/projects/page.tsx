 ...
 * Optimized for high-density visual monitoring with responsive breakpoints.
 */
    ...
    <div className="space-y-8 sm:space-y-12 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 sm:gap-8">
        <div className="space-y-1">
          <h1 className="text-2xl sm:text-4xl font-bold font-headline tracking-tight text-slate-900 leading-tight">Projects Registry</h1>
          <p className="text-xs sm:text-sm text-slate-500 font-medium tracking-normal mt-2">
        ...
        <div className="flex items-center gap-3 sm:gap-4 w-full md:w-auto">
          <div className="flex items-center bg-white p-1 rounded-full shadow-sm border border-slate-100 shrink-0">
            <Button 
              ...
              className={`h-9 w-9 sm:h-11 sm:w-11 rounded-full transition-all ${viewMode === 'grid' ? 'bg-slate-100 text-primary shadow-inner' : 'text-slate-400'}`}
            >
            ...
            <Button 
              ...
              className={`h-9 w-9 sm:h-11 sm:w-11 rounded-full transition-all ${viewMode === 'list' ? 'bg-slate-100 text-primary shadow-inner' : 'text-slate-400'}`}
            >
      ...
      {pipelineProjects.length > 0 && (
        <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-500">
          <div className="flex items-center gap-4 px-2 sm:px-4">
            ...
            <div>
              <h3 className="text-xl sm:text-2xl font-bold font-headline text-slate-900 tracking-tight leading-tight">Pipeline Sync</h3>
              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Active Discussion Entities</p>
            </div>
          </div>
              ...
              <Card key={p.id} className="border-none shadow-2xl shadow-slate-200/50 rounded-[2.5rem] sm:rounded-[3rem] bg-white overflow-hidden group hover:-translate-y-2 transition-all duration-500 border border-slate-50 h-56 sm:h-64 flex flex-col justify-between p-8 sm:p-10">
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex justify-between items-start">
                    ...
                    <Target className="h-4 w-4 sm:h-5 sm:w-5 text-blue-200 opacity-20" />
                  </div>
                  <div>
                    <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{p.clientName || clientMap.get(p.clientId)}</p>
                  ...
                  <div className="p-3 sm:p-4 flex-grow">
                    <div className="relative aspect-[4/3] overflow-hidden rounded-[2rem] sm:rounded-[2.2rem] shadow-sm">
                      ...
                      <div className="absolute top-4 sm:top-6 left-4 sm:left-6">
                        <Badge className="bg-white/90 backdrop-blur-md text-slate-900 border-none font-bold text-[7px] sm:text-[8px] uppercase px-3 sm:px-4 py-1.5 rounded-full shadow-lg tracking-widest">
                          {project.status || "DISCUSSION"}
                        </Badge>
                      </div>
                      <div className="absolute bottom-4 sm:bottom-6 left-6 sm:left-8 right-6 sm:right-8">
                        <div className="flex items-center gap-2 mb-1">
                          <p className="text-[8px] sm:text-[10px] font-bold text-white/80 uppercase tracking-widest truncate">{clientMap.get(project.clientId)}</p>
                          <CheckCircle2 className="h-3 w-3 text-green-400 fill-green-400/20" />
                        </div>
                        <h3 className="text-xl sm:text-2xl font-bold font-headline text-white tracking-tight leading-tight line-clamp-2">{project.name}</h3>
                      </div>
                    </div>
                      ...
                      <div className="pt-4 sm:pt-6 border-t border-slate-50 flex items-center justify-between">
                        <div className="flex -space-x-2 sm:-space-x-3">
                          {project.crew?.slice(0, 3).map((member: any, i: number) => (
                            <Avatar key={i} className="h-8 w-8 sm:h-10 sm:w-10 border-2 sm:border-4 border-white shadow-sm rounded-xl">
                              <AvatarImage src={member.thumbnail} />
                              <AvatarFallback className="text-[8px] font-bold bg-slate-50">{member.name?.[0]}</AvatarFallback>
                            </Avatar>
                          ))}
                        </div>
                ...
                <Table>
                  <TableHeader className="bg-slate-50/50">
                    <TableRow className="hover:bg-transparent border-slate-100">
                      <TableHead className="px-6 sm:px-10 py-5 sm:py-6 text-[10px] font-bold uppercase tracking-widest text-slate-400">Production Entity</TableHead>
                      <TableHead className="text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center hidden sm:table-cell">Phase</TableHead>
                      <TableHead className="text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center">Crew</TableHead>
                      <TableHead className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Budget (INR)</TableHead>
                      <TableHead className="text-right px-6 sm:px-10 text-[10px] font-bold uppercase tracking-widest text-slate-400">Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {productionProjects.map((p) => (
                      <TableRow key={p.id} className="group hover:bg-slate-50/50 transition-colors border-slate-50">
                        <TableCell className="px-6 sm:px-10 py-6 sm:py-8">
                          <div>
                            <div className="flex items-center gap-3">
                              <h4 className="font-bold text-base sm:text-xl text-slate-900 tracking-tight leading-tight line-clamp-1">{p.name}</h4>
                              <CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-green-500 hidden xs:block" />
                            </div>
                            <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-widest line-clamp-1">
                              {clientMap.get(p.clientId)} • <span className="text-primary/60">#{p.id.substring(0, 6).toUpperCase()}</span>
                            </p>
                          </div>
                        </TableCell>
                        <TableCell className="text-center hidden sm:table-cell">
                          <Badge className="rounded-full px-4 sm:px-5 py-1 sm:py-1.5 text-[8px] sm:text-[9px] font-bold uppercase border-none tracking-widest bg-slate-100 text-slate-500 whitespace-nowrap">
                            {p.status || "DISCUSSION"}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center -space-x-2">
                            {p.crew?.slice(0, 3).map((m: any, idx: number) => (
                              <Avatar key={idx} className="h-7 w-7 sm:h-8 sm:w-8 border-2 border-white rounded-lg shadow-sm">
