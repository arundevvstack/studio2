 ...
 * Enhanced with mobile-friendly adaptive filters and responsive grid.
 */
      ...
      <main className="flex-1 space-y-6 sm:space-y-8 overflow-auto pb-20">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-xs sm:text-sm text-slate-500 font-medium tracking-normal mt-2 sm:mt-3">Manage creative professionals across Kerala hubs.</p>
          </div>
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button className="h-12 sm:h-14 flex-1 sm:flex-none px-8 sm:px-10 rounded-full font-bold bg-primary hover:bg-primary/90 text-white shadow-2xl shadow-primary/30 gap-2 tracking-widest transition-all active:scale-95 text-[10px] sm:text-xs uppercase">
                  <Plus className="h-4 w-4 sm:h-5 sm:w-5" /> Add Talent
                </Button>
              </DialogTrigger>
        ...
        <div className="space-y-6">
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 p-4 sm:p-6 bg-white/50 rounded-[2rem] border border-slate-100 shadow-sm animate-in fade-in duration-300">
              <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mr-2">Filters:</span>
        ...
        {isLoading ? (
                ...
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100">
                      ...
                      <th className="px-6 py-5 sm:py-6 text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center">Tier</th>
                      <th className="px-6 py-5 sm:py-6 text-[10px] font-bold uppercase tracking-widest text-slate-400">Hub</th>
                      <th className="px-6 py-5 sm:py-6 text-[10px] font-bold uppercase tracking-widest text-slate-400 text-center">Load</th>
                      ...
                      <tr key={t.id} className="group hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 sm:px-10 py-5 sm:py-6">
                          <div className="flex items-center gap-4 sm:gap-5">
                            ...
                            <div>
                              ...
                              <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{t.category}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5 sm:py-6 text-center">
                          <Badge className={`border-none font-bold text-[8px] sm:text-[9px] uppercase px-3 sm:px-4 py-1 rounded-full tracking-widest ${t.paymentStage === 'Yes' ? 'bg-green-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                        ...
                        <td className="px-6 sm:px-10 py-5 sm:py-6 text-right">
                          <Button asChild variant="ghost" size="sm" className="h-9 sm:h-10 rounded-full px-4 sm:px-6 font-bold text-[9px] sm:text-[10px] uppercase gap-2 hover:bg-primary hover:text-white transition-all">
                            <Link href={`/shoot-network/${t.id}`}>Brief <ArrowRight className="h-3.5 w-3.5" /></Link>
                          </Button>
                        </td>
                      </tr>
                    ))}
          ...
          <div className="py-24 sm:py-40 flex flex-col items-center justify-center border-2 border-dashed border-slate-100 rounded-[2.5rem] sm:rounded-[4rem] bg-white/30 text-center space-y-6 sm:space-y-8 px-6">
            ...
            <div className="space-y-2">
              <p className="text-lg sm:text-xl font-bold text-slate-400 uppercase tracking-widest">No Professionals Found</p>
              <p className="text-xs sm:text-sm text-slate-300 italic tracking-normal max-w-xs mx-auto">Adjust your filters or initiate a new talent onboarding process.</p>
              <Button onClick={() => setFilters({ category: [], district: "All", gender: "All", paymentStage: "All", tags: [] })} variant="link" className="mt-4 text-primary font-bold text-xs uppercase tracking-widest p-0">Clear filters</Button>
            </div>
          </div>
        )}
      </main>
