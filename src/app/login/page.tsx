...
/**
 * @fileOverview High-Fidelity Identity Governance Portal.
 * Design inspired by modern illustrative aesthetic with floating UI elements.
 */
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#f0f2f5] flex items-center justify-center p-4 sm:p-10 font-body">
      <div className="max-w-[1200px] w-full bg-white rounded-[3rem] shadow-[0_30px_80px_rgba(0,0,0,0.08)] overflow-hidden flex flex-col lg:flex-row h-full min-h-[750px] border border-slate-100">
        
        {/* Left Side: Auth Form */}
        <div className="flex-1 p-10 sm:p-20 flex flex-col justify-center bg-gradient-to-br from-slate-50/50 to-white">
        ...
        {/* Right Side: Illustrative Design */}
        <div className="hidden lg:flex flex-1 relative bg-slate-950 overflow-hidden items-center justify-center p-12">
          ...
          <div className="relative w-full h-full max-w-lg">
            ...
            <div className="absolute top-[10%] right-[-5%] bg-yellow-400 p-4 rounded-2xl shadow-2xl animate-in slide-in-from-right-8 duration-700 delay-300">
            ...
            <div className="absolute bottom-[20%] left-[-10%] bg-white/10 backdrop-blur-xl border border-white/10 p-6 rounded-[2.5rem] shadow-2xl animate-in slide-in-from-left-8 duration-700 delay-500">
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  {[22, 23, 24, 25, 26].map((day, i) => (
                    <div key={day} className={`h-10 w-10 rounded-xl flex flex-col items-center justify-center transition-all ${i === 2 ? 'bg-primary text-white scale-110 shadow-lg' : 'bg-white/5 text-white/40'}`}>
            ...
            <div className="absolute top-[40%] right-[10%] bg-white p-5 rounded-3xl shadow-2xl max-w-[200px] animate-in zoom-in-95 duration-1000 delay-700">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <div className="h-2 w-2 rounded-full bg-accent" />
                  <p className="text-[10px] font-bold text-slate-900 uppercase tracking-widest">Daily Brief</p>
                </div>
                <p className="text-[11px] font-medium text-slate-500 leading-relaxed">Coordinate drone shots for Kerala Tourism campaign.</p>
                <div className="flex -space-x-2">
                  {[1, 2, 3].map(i => (
                    <Avatar key={i} className="h-6 w-6 border-2 border-white rounded-lg">
                      <AvatarImage src={`https://picsum.photos/seed/crew${i}/100/100`} />
                    </Avatar>
                  ))}
                  <div className="h-6 w-6 rounded-lg bg-slate-50 flex items-center justify-center border-2 border-white text-[8px] font-bold text-slate-400">+2</div>
                </div>
              </div>
            </div>
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center space-y-6">
                <div className="inline-block p-4 rounded-[2rem] bg-white/5 backdrop-blur-md border border-white/10 shadow-2xl mb-4">
