  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-full animate-in fade-in duration-500">
      <div className="lg:col-span-8 space-y-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-0.5">
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold font-headline tracking-normal text-slate-900 dark:text-white leading-tight">Workspace Hub</h1>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-normal">Pipeline performance</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex flex-col items-end">
              ...
              <div className="flex items-center gap-3 mt-1">
                <Progress value={stats.percent} className="h-1 w-20 bg-slate-100 dark:bg-slate-800" />
                <span className="text-[10px] font-bold text-slate-900 dark:text-white tracking-normal">{stats.percent}%</span>
              </div>
            </div>
            <Button asChild size="icon" className="h-9 w-9 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 shadow-sm">
        ...
        {/* High-Density Project Grid: 4 in row, max 4, reduced height */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {projectsLoading ? (
            <div className="col-span-full h-40 flex items-center justify-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            featuredProjects.map((project, idx) => (
              <Card key={project.id} className={`${idx % 2 === 0 ? 'bg-slate-900' : 'bg-primary'} border-none shadow-lg rounded-[1.5rem] overflow-hidden relative group cursor-pointer`}>
                <CardContent className="p-5 text-white h-44 flex flex-col justify-between">
                  ...
                  <div className="absolute right-0 bottom-0 opacity-10 group-hover:scale-110 transition-transform pointer-events-none">
            ...
            <Card className="col-span-full border-2 border-dashed border-slate-100 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 rounded-[2rem] h-40 flex flex-col items-center justify-center space-y-3">
              <div className="h-10 w-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center p-2.5">
                <Grid className="h-full w-full text-slate-200 dark:text-slate-700" />
              </div>
        ...
        <Card className="border-none shadow-sm rounded-[2.5rem] bg-white dark:bg-slate-900 p-10">
          <CardHeader className="p-0 mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              ...
              <h3 className="text-xl font-bold font-headline text-slate-900 dark:text-white tracking-normal mt-1">Revenue & Pipeline</h3>
            </div>
            <div className="flex items-center gap-4">
              <Tabs value={forecastView} onValueChange={setForecastView} className="bg-slate-50 dark:bg-slate-800 p-1 rounded-xl">
                <TabsList className="bg-transparent h-8 gap-1 p-0">
                  <TabsTrigger value="weekly" className="text-[10px] font-bold uppercase px-4 rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all tracking-normal">Weekly</TabsTrigger>
                  <TabsTrigger value="monthly" className="text-[10px] font-bold uppercase px-4 rounded-lg data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all tracking-normal">Monthly</TabsTrigger>
                </TabsList>
              </Tabs>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold font-headline tracking-normal text-slate-900 dark:text-white">Strategic Items</h2>
            <div className="flex items-center gap-2">
              <Button variant="outline" className="rounded-xl px-6 text-xs font-bold bg-white/50 dark:bg-slate-900/50 border-slate-100 dark:border-slate-800 shadow-sm tracking-normal">Archive</Button>
          ...
          <div className="space-y-4">
            {filteredProjects.length > 0 ? (
              filteredProjects.slice(0, 5).map((project) => (
                <div key={project.id} className="flex items-center justify-between p-6 bg-white dark:bg-slate-900 rounded-3xl border border-slate-50 dark:border-slate-800 shadow-sm hover:shadow-md transition-all group cursor-pointer">
                  <div className="flex items-center gap-6">
                    <div className="h-12 w-12 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center shrink-0">
                      <Briefcase className="h-5 w-5 text-slate-300" />
                    </div>
                    <div>
                      <p className="font-bold text-base text-slate-900 dark:text-white tracking-normal leading-none">{project.name}</p>
                  ...
                  <div className="flex items-center gap-10">
                    <div className="text-right">
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white tracking-normal">₹{(project.budget || 0).toLocaleString('en-IN')}</p>
                    </div>
                    <Button asChild variant="ghost" size="icon" className="h-10 w-10 rounded-xl bg-slate-50 dark:bg-slate-800 group-hover:bg-primary group-hover:text-white transition-colors">
                      <Link href={`/projects/${project.id}`}>
                        <ChevronRight className="h-4 w-4" />
                      </Link>
                    </Button>
              ...
              <div className="p-20 text-center bg-white/50 dark:bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-100 dark:border-slate-800">
      ...
      <div className="lg:col-span-4 space-y-8">
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-sm border border-slate-50 dark:border-slate-800 space-y-8">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-lg tracking-normal text-slate-900 dark:text-white">Throughput Stats</h3>
          ...
          <div className="grid grid-cols-2 gap-8">
            <div>
               ...
               <p className="text-3xl font-bold font-headline mt-1 tracking-normal text-slate-900 dark:text-white">{stats.completed}</p>
            </div>
            <div>
            ...
            <div>
               ...
               <div className="h-10 w-10 rounded-2xl bg-primary/10 text-primary border-2 border-white dark:border-slate-800 flex items-center justify-center text-xs font-bold mt-2 shadow-sm">
            ...
            <div className="col-span-2 pt-4 border-t border-slate-50 dark:border-slate-800">
               ...
               <div className="flex items-center gap-2 mt-2">
                 ...
                 <p className="text-2xl font-bold font-headline text-slate-900 dark:text-white tracking-normal">
        ...
        <div className="space-y-6">
           <h2 className="text-xl font-bold font-headline tracking-normal text-slate-900 dark:text-white">Quick Strategy</h2>
